...
public class Testcomplex_hello_puml {
	...
	public void test() throws Exception {
		...
		if(eval("${testCondition} == 'SomeValue'")) {
			paramsMap.put("variableName", "value");
			paramsMap.put("variableName2", "value2");
			
			Response roundtrip1 = RestAssured.given()
			        .auth().basic(subst("${.username}"), subst("${.password}"))
			        .param("variableName", subst("${variableName}"))
			        .param("variableName2", subst("${variableName2}"))
			    .when()
			        .post(subst("${.path}") + subst("/hello/${id}"))
			    .then()
			        .assertThat()
			            .statusCode(IsIn.isIn(Arrays.asList(200)))        
			        	.and().extract().response();
			paramsMap.put("variableName", roundtrip1.jsonPath().getString("itemA"));
			paramsMap.put("variableName2", roundtrip1.jsonPath().getString("itemB"));
		}
	}
